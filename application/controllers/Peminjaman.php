...
class Peminjaman extends CI_Controller
{
    function __construct()
    {
        ...
        if ($this->session->userdata('status') != "login") {
            $alert = $this->session->set_flashdata('alert', 'Anda belum
Login');
            redirect(base_url());
        }
    }
    ...
    {
        $data['anggota'] =
            $this->M_perpus->edit_data(array('id_anggota' =>
    ...
    public function selesai_booking($where)
    {
        ...
        $data['useraktif'] =
            $this->M_perpus->edit_data(array('id_anggota' =>
            $this->session->userdata('id_agt')), 'anggota')->result();
        $data['items'] = $this->db->query("SELECT * from transaksi T,detail_pinjam D, buku B where T.id_buku=D.id_buku and T.id_pinjam=D.id_pinjam and T.id_anggota='$where'")->result();
        $this->load->view('desain');
        $this->load->view('toplayout', $data);
